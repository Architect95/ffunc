...
test_lengths     <- c(-2*len, -(len+1):(len+1), 2*len)
...
# Native multibyte:
# Here str_sub produces a different output so it is not used in the benchmark
...
test_len    <- 100  # This is assumed to be longer than the lookback length
test_string <- paste(rep(test_string, times= test_len %/% nchar(test_string)),
                     collapse = "")

test_strings     <- rep(test_string, times = benchmark_length/1000)
