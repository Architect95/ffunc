...
static FORCE_INLINE void warnIfVectorArgRecycled(
  const char * restrict arg_name, const R_xlen_t arg_len, 
  const R_xlen_t len_to_match);
static FORCE_INLINE CharLenCE substrUtf8(
...
static FORCE_INLINE CharLenCE signedSubstrUtf8(
  const char *str, const int nbytes, const cetype_t encoding,
  const int i_start, const int i_stop, const bool skip_validity_check);
static FORCE_INLINE CharLenCE substrSingleByteChars(
  const char *str, const int nbytes, const cetype_t encoding,
  const int i_start, const int i_stop);
static FORCE_INLINE CharLenCE substrMultiByteChars(
  const char *str, const int nbytes, const cetype_t encoding,
  const int i_start, const int i_stop);
...
static FORCE_INLINE CharLenCE
substrAfterShiftStateIsSet(const char *rest_of_sub, const cetype_t encoding, 
  mbstate_t mb_st, const char *const end, const int cur_count, const int i_stop, 
...
substrAfterShiftStateIsSetUnguarded(
  const char *rest_of_sub, const cetype_t encoding, 
  mbstate_t mb_st, const char *const end, const int cur_count, const int i_stop,
  const wchar_t first_substr_char) {
...
substrMultiByteChars(const char *const start_of_str, const int nbytes,
  ...
  StrIndex start_of_sub = mbCount(j, just_before_start, str, end, &mb_st);
  j   = start_of_sub.index;
  str = start_of_sub.ptr;
  
  if (!mbsinit(&mb_st)) {  
    return substrWithinShiftState(str, encoding, mb_st, end, j, i_stop);
  }
  ...
  if (i_stop >= nbytes) {
    ...
    StrIndex end_of_sub = mbCount(j, i_stop, str, end, &mb_st);
